...
void solve() {
    ...
    unique_ptr<SegTree> c_segt = make_unique<SegTree>(0, n + 1, vector<int>(n + 2, 0));
    for (int i = 0; i < m; i++) {
        int l, r;
        cin >> l >> r;
        ...
        L[i] = {l, r};
        R[i] = {r, l};
        c_segt->rangeAddUpdate(l, r, 1);
    }

    sort(L.begin(), L.end(), greater<pair<int, int>>());
    sort(R.begin(), R.end());

    // unique_ptr<SegTree> l_segt = make_unique<SegTree>(0, n + 1, vector<int>(n + 2, INF32));
    unique_ptr<SegTree> r_segt = make_unique<SegTree>(0, n + 1, vector<int>(n + 2, -INF32));
    for (int i = 0; i < m; i++) {
        // l_segt->rangeSetUpdate(L[i].fi, L[i].se, L[i].fi);
        r_segt->rangeSetUpdate(R[i].se, R[i].fi, R[i].fi);
    }

    // for (int i = 1; i <= n; i++) 
    //     cout << i << ": " << c_segt->rangeSumQuery(i, i) << ' ' << l_segt->rangeSumQuery(i, i) << ' ' << r_segt->rangeSumQuery(i, i) << endl;
    // cout << endl;

    vector<int> memo(n + 2);
    for (int i = n; i >= 1; i--) {
        int cnt = c_segt->rangeSumQuery(i, i);
        // int l = l_segt->rangeSumQuery(i, i);
        int r = r_segt->rangeSumQuery(i, i);

        memo[i] = max(memo[i], memo[i + 1]);
        if (cnt != 0)
            memo[i] = max(memo[i], memo[r + 1] + cnt);
    }
