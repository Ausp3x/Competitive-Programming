...
lng ModInt::MOD = 1'000'000'000'000'000'009;
...
struct FenTree {
    ...
    void pointAddUpdate(int i_cur, int j_cur, ModInt del) {
        assert(D == 2);
    ...
    ModInt prefixSumQuery(int i_cur, int j_cur) const {
        assert(D == 2);
        ModInt sum = 0;
    ...
    ModInt rangeSumQuery(int x1, int y1, int x2, int y2) const {
        assert(D == 2);
        if (x1 > x2 || y1 > y2) {
            return 0;
        }
...
void solve() {
    ...
    for (int i = 1; i <= q; i++) {
        ...
        if (type == 1) {
            ...
            ModInt hash = ModInt(2).pow(i);
            fn.pointAddUpdate(x,     y,     hash);
            fn.pointAddUpdate(x,     Y + 1, -hash);
            fn.pointAddUpdate(X + 1, y,     -hash);
            fn.pointAddUpdate(X + 1, Y + 1, hash);
        } else if (type == 2) {
            int j;
            cin >> j;
            ...
            ModInt s1 = fn.rangeSumQuery(1, 1, x1, y1);
            ModInt s2 = fn.rangeSumQuery(1, 1, x2, y2);
