...
void solve() {
    ...
    vector<lng> C(n + 2);
    set<pair<int, int>> S;
    unique_ptr<SegTree> segt = make_unique<SegTree>(0, n - 1, vector<int>(n));
    for (int k = n + 1; k >= 1; k--) {
        while (!V.empty()) {
            auto [y, x, i] = V.back(); 
            if (y >= k) {
        ...
        lng cur = segt->rangeSumQuery(0, n - 1);
