...
namespace FastMoly {
    ...
    static constexpr uint IMOD = 998244351;
    static constexpr uint RSQ = 932051910;
    
    inline uint red(ulng x) {
        uint q = uint(x) * IMOD, res = (x + ulng(q) * MOD) >> 32;
        return res - (res >= MOD) * MOD;}
    inline uint init(uint a) { return red(ulng(a) * RSQ); }
    inline uint mul(uint a, uint b) { return red(ulng(a) * b); }
    inline uint pow(uint a, uint b) {
        uint res = 1;
        while (b > 0) {
            if (b & 1) { res = ulng(res) * a % MOD; }
            a = ulng(a) * a % MOD;
            b >>= 1;}
        return res;}
    inline __m256i vred(__m256i x0, __m256i x1) {
    ...
    inline void calc(int n) {
        ...
        if (rt.size() < n) {
            ...
            for (int len = m; len < n; len <<= 1) {
                uint w0 = init(pow(3, (MOD - 1) / (2 * len)));
                uint iw0 = init(pow(332748118, (MOD - 1) / (2 * len)));
                for (int i = len / 2; i < len; i++) {
                    rt[2 * i]  = rt[i];  rt[2 * i + 1]  = mul(rt[i], w0);
                    irt[2 * i] = irt[i]; irt[2 * i + 1] = mul(irt[i], iw0);}}}}
    template<typename T>
    inline void ntt(vector<T> &a, bool is_inv) {
        int n = a.size(); calc(n);
        uint *A = reinterpret_cast<uint*>(a.data());
