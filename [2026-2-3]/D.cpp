...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        cin >> B[i];
    vector<pair<lng, int>> D = {{0, -1}}, P = {{0, -1}};
    for (int i = 1; i <= m; i++) {
        int t;
        cin >> t;
        if (t == 1) {
    ...
    unique_ptr<SegTree> A_seg = make_unique<SegTree>(1, n, A);
    unique_ptr<SegTree> B_seg = make_unique<SegTree>(1, n, B); 
    ...
    while (r - l > 1) {
        ...
        auto [a_mn, d_a] = A_seg->rangeMinQuery(1, md);
        auto [b_mn, d_b] = B_seg->rangeMinQuery(1, md);

        bool valid = false;
        for (int i = 0; i <= k; i++) {
            int j = k - i;

            if (i >= D.size() || j >= P.size())
                continue;

            if (D[i].fi * a_mn + P[j].fi * b_mn > s)
                continue;

            // cout << md << ' ' << i << ' ' << j << ' ' << i + j << endl;
