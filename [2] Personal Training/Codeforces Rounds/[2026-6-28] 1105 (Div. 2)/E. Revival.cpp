...
void solve(int t) {
    ...
    lng n;
    cin >> n;
    vector<pair<char, lng>> A(n + 1);
    vector<vector<lng>> groups = {{}};
    for (lng i = 1; i <= n; i++) {
        cin >> A[i].fi >> A[i].se;
        groups.back().pb(A[i].se);
        if (A[i].fi == 's') {
            groups.back().back() *= -1;
            groups.pb({});
        }
    ...
    vector<lng> ans(n + 1);
    while (!groups.empty()) {
        vector<lng> grp = groups.back();
        groups.pop_back();
        
        if (grp.back() > 0) {
            for (lng x : grp | views::reverse) {
                R.erase(x);
                ans[cnt] = x;
                cnt--;
            }
        ...
        lng len = grp.size();
        indexed_set<lng> fixed;
        for (lng i = len - 2; i >= 0; i--) {
            R.erase(grp[i]);
            K += R.size() - R.order_of_key(grp[i]);
            fixed.insert(grp[i]);
        }
        ...
        while (r - l > 1) {
            ...
            lng cur = *R.find_by_order(md);
            lng sum = md + 1 + 2LL * fixed.order_of_key(cur);
